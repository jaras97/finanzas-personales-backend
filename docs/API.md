...
| GET | `/transactions/with-category` | Query `startDate`, `endDate`, `categoryId`, `type`, `source` (`account`→`debt_id IS NULL`, `credit_card`→`debt_id IS NOT NULL`), `uncategorized` (default `False`), `page`, `page_size` (máx 100), `include_reversals` (default `False`). → `{items, total, page, page_size, totalPages}`. `uncategorized=true` aplica el filtro completo de «pendiente» (ver la fila siguiente); no es un `categoryId` más porque «sin clasificar» son **dos** estados en la base y un id solo expresa uno. |
| GET | `/transactions/uncategorized/count` | → `{count}`. Movimientos sin clasificar **y editables**: `category_id IS NULL` o apuntando a la hoja de sistema `uncategorized` (donde deja la importación de CSV lo que ninguna regla resuelve), no cancelados, no reversas, `source_type IS NULL`, tipo `income`/`expense`. **Sin rango de fechas a propósito**: un pendiente de marzo tiene que avisar aunque el usuario esté mirando septiembre. Incluir lo no editable daría un contador que nunca baja. |
| PATCH | `/transactions/bulk-category` | `{transaction_ids: [int], category_id}` → `{updated, skipped: [{id, reason}]}`. Máximo 500 por llamada. La categoría se valida una vez (hoja, activa, del usuario → 400); la elegibilidad se comprueba **por fila** y lo inelegible se omite con su motivo en vez de abortar la operación entera. Los ids ajenos se reportan como `No encontrado` y no se tocan. |
| PATCH | `/transactions/{id}` | `{description?, category_id?, date?}` (al menos uno). Solo permite editar transacciones manuales `income`/`expense`: bloqueado si está cancelada, si es ella misma una reversión, o si tiene `source_type` (cualquier transacción auto-generada es inmutable). `category_id` pasa por `exigir_hoja`: un grupo se rechaza con 400 igual que al crear (invariante I1). |

...
- [ ] Subcategorías (`parent_id`): **decidido aplazar** el 2026-09-03. Es lo más caro del PDF (toca presupuestos, reglas, resúmenes, import CSV y toda la agregación) y resuelve una saturación que nadie tiene: la mediana medida es de 4-6 categorías por usuario, y solo uno llegó a 29. Retomar cuando alguien sature de verdad la lista plana.
- [ ] El icono de categoría se guarda (`category.icon`, nombres de lucide) pero el frontend **todavía no lo pinta**: solo usa el color. Falta un mapa nombre→componente con fallback. Esfuerzo: S.

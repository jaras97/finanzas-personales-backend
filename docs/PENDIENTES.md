...
- [x] ~~Taxonomía de categorías del PDF (Fases 1 y 2)~~ ✅ 2026-09-04 — contrastada primero contra producción: el **92%** de los 1.447 movimientos reales ya caía en esa estructura, y de lo que quedaba fuera, 109 de 114 eran de **una sola persona** llevando un negocio por la app. Conclusión: el valor está en el onboarding, no en arreglar datos. Se siembran **13 categorías del núcleo** al registrarse (`app/utils/default_categories.py`), con `color` e `icon`; las 25 completas se ofrecen bajo demanda en `POST /categories/suggested`, que **solo añade lo que falta** comparando sin tildes ni mayúsculas y nunca renombra, fusiona ni borra. No son `is_system`: el usuario las controla. Arregla de paso que los colores de los gráficos se asignaban por posición y una categoría cambiaba de color entre meses. 11 tests backend + 6 frontend.
- [x] ~~Subcategorías (Fase 3)~~ ✅ 2026-09-04 — se había aplazado el 2026-09-03 por coste/beneficio (mediana de 4-6 categorías por usuario); se implementó por decisión del usuario. **Dos niveles** (`category.parent_id`), tipo heredado del padre, nombre único **por padre** (para que «Transporte › Gasolina» y «Viajes › Gasolina» convivan). Reglas que impiden corromper el árbol: no hay tercer nivel, no se mezclan tipos, no se puede ser madre de sí misma, un padre con hijas no puede volverse subcategoría, y no se desactiva un padre con hijas activas. **Los reportes suman la hija al padre** (`/summary`) y **un presupuesto del padre incluye a sus hijas** — sin eso mostraría plata disponible que ya se gastó. 14 tests + 7 de frontend.
- [x] ~~El icono de categoría no se pintaba~~ ✅ 2026-09-04 — `lib/categoryIcon.tsx` mapea los 25 nombres sembrados a componentes de lucide, con `Tag` como fallback para cualquier nombre desconocido. El mapa es explícito y no un import dinámico: meter todo lucide en el bundle por 25 iconos no se paga. Selector de icono en `CategoryModal`.
- [ ] Las subcategorías solo se agrupan al padre en `/summary`. `summary_extra` (patrimonio, pasivos) no agrupa por categoría, así que no le aplica, pero **el import de CSV y las reglas de categorización tratan una subcategoría como cualquier otra**: se pueden asignar y funcionan, simplemente no hay una regla que diga "todo lo de este padre". Suficiente por ahora; revisar si alguien lo pide.
